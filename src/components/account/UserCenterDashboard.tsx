...
import { ArrowRight, Clock3, Download, FileClock, Heart, LayoutDashboard, ShieldCheck, Sparkles, Upload } from 'lucide-react'
...
export function UserCenterDashboard({ name, dashboard }: { name: string; dashboard: UserCenterData }) {
  ...
  return (
    <div className="space-y-8">
      <section className="flex flex-col gap-5 rounded-[var(--radius)] border border-[var(--border)] bg-gradient-to-br from-blue-50 via-white to-sky-50 p-6 shadow-[var(--shadow)] md:flex-row md:items-center md:justify-between md:p-8">
        <div><p className="text-sm font-medium text-[var(--brand)]">个人中心</p><h1 className="mt-2 text-3xl font-bold tracking-tight">你好，{name}</h1><p className="mt-2 text-slate-600">查看你的 Skill 审核进度与创作数据。</p></div>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.55fr_1fr]">
        <div className="rounded-[var(--radius)] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow)] md:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">我的 Skills</h2><p className="mt-1 text-sm text-slate-500">查看你的投稿、审核状态与发布数据。</p></div><Link href="/me/submit-skill" className="inline-flex items-center gap-1 text-sm font-medium text-[var(--brand)] hover:underline">新建投稿 <ArrowRight className="size-4" /></Link></div>
        ...
        <div className="space-y-6">
          <section className="rounded-[var(--radius)] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow)]"><div className="flex items-start gap-3"><span className={`flex size-10 shrink-0 items-center justify-center rounded-lg ${dashboard.permission.active ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}><ShieldCheck className="size-5" /></span><div><h2 className="font-bold">投稿资格</h2><p className={`mt-1 text-sm ${dashboard.permission.active ? 'text-emerald-700' : 'text-amber-700'}`}>{dashboard.permission.active ? `已开通，至 ${new Date(dashboard.permission.expiresAt!).toLocaleDateString('zh-CN')} 有效` : '尚未开通投稿资格'}</p><p className="mt-2 text-sm text-slate-500">{dashboard.permission.active ? '可以提交新的 Skill 或新版本，提交后需等待审核。' : '请先提交投稿资格申请，审核通过后即可投稿。'}</p></div></div></section>
          <section className="rounded-[var(--radius)] border border-[var(--border)] bg-white p-5 shadow-[var(--shadow)]"><h2 className="font-bold">我的快捷入口</h2><div className="mt-3 divide-y divide-[var(--border)]"><QuickLink href="/me/favorites" icon={Heart} title="我的收藏" detail="查看已收藏的 Agent" /><QuickLink href="/me/downloads" icon={Download} title="我的下载" detail="查看下载过的版本" /><QuickLink href="/me" icon={LayoutDashboard} title="账户设置" detail="修改昵称和查看邮箱" /></div></section>
