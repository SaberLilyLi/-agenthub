...
def _load_archetype_templates() -> tuple[str, str]:
    ...
    if not content:
        return (
            "你是协调者，负责规划、委派并确保质量。所有面向 CEO 和界面的回复默认使用简体中文。",
            "你是执行者，负责产出满足验收标准的高质量交付物。所有面向 CEO 和界面的回复默认使用简体中文。",
        )
...
def build_role_identity(employee_id: str) -> str:
    ...
    header = (
        f"## 身份与岗位\n"
        f"你是 {name}{nick_str}，岗位为{level_label}{role}{dept_str}。\n"
        f"所有面向 CEO、员工和界面的回复默认使用简体中文；CEO、API、Token、URL、ID、工具名和模型名保持原样。\n"
    )
    if is_manager:
...
async def _summarize_project_for_ceo(
    ...
    for node in work_nodes:
        status = "成功" if node.status != TaskPhase.FAILED.value else "失败"
    ...
    raw_context = "\n\n---\n\n".join(raw_parts)
    deliverables_ctx = "\n".join(f"  - {f}" for f in deliverables) if deliverables else "（无）"

    prompt = (
        f"你是执行助理。请为 CEO 撰写简洁的中文项目完成报告。\n\n"
        f"项目：{project_name}\n\n"
        f"交付物：\n{deliverables_ctx}\n\n"
        f"原始任务结果：\n{raw_context}\n\n"
        f"要求：\n"
        ...
        f"- 突出关键交付物及其质量\n"
        f"- 说明需要 CEO 关注的失败或问题\n"
        f"- 必须使用简体中文，CEO、API、Token、URL、ID、工具名和模型名保持原样\n"
        f"- 不要包含问候语或署名\n"
        f"- 直接、客观"
    )
        ...
        if node.status != TaskPhase.HOLDING.value:
            ...
            if project_id:
                ...
                if not is_system_project_id(project_id) and node.result:
                    ...
                    append_action(project_id, employee_id, f"{role} 任务已完成", summary)
    ...
    async def _on_child_complete_inner(self, employee_id: str, entry: ScheduleEntry, project_id: str = "") -> None:
        ...
        if node.node_type not in SKIP_COMPLETION_TYPES and tree.is_project_complete():
            ...
            if has_unresolved_ceo_request(tree.get_children(ea_node.id), _CEO_ID):
                ...
                lines = [f"✅ 项目已完成：{project_name}", ""]
                lines.append(f"📊 结果：{succeeded}/{total} 个任务成功" + (f"，{failed} 个失败" if failed else ""))
                if _elapsed:
                    lines.append(f"⏱ 耗时：{_elapsed}")
                if _proj_cost > 0:
                    lines.append(f"💰 成本：${_proj_cost:.2f}")
                if deliverables:
                    lines.append(f"\n📁 交付物（{len(deliverables)} 个文件）：")
                    for fname in deliverables[:10]:
                        lines.append(f"  • {fname}")
                    if len(deliverables) > 10:  # pragma: no cover — >10 deliverables edge case
                        lines.append(f"  ……另有 {len(deliverables) - 10} 个文件")  # pragma: no cover
                if ea_summary:
                    lines.append(f"\n📝 摘要：\n{ea_summary}")
                lines.append("\n👉 回复以确认完成，或说明修改要求以创建新迭代。")
                confirm_desc = "\n".join(lines)
                ...
                _confirm_prompt = f"✅ {project_name} 已完成。请回复确认，或说明修改要求以创建新迭代。"
