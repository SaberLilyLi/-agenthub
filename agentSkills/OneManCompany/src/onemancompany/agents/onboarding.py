...
async def execute_hire(
    ...
    *,
    ...
    # Runtime launch scripts are opt-in. A company-hosted talent may include a
    # launch.sh as package metadata, but it must not bypass the company LLM.
    if talent_dir and hosting in (HostingMode.SELF, HostingMode.OPENCLAW):
    ...
    if not remote:
        ...
        if not get_agent_loop(emp_num):
            ...
            elif hosting == HostingMode.OPENCLAW and (emp_dir / LAUNCH_SCRIPT).exists():
                # launch.sh is reserved for employees explicitly hosted by OpenClaw.
                # Company-hosted employees always use EmployeeAgent so they inherit
                # the company's provider/model and server-side credentials.
