...
class ChatPanel {
        ...
        this._container.innerHTML = `
            <div class="chat-panel">
                <div class="chat-panel-header">
                    ...
                    <button class="chat-panel-clear-btn">清空</button>
                    <button class="chat-panel-close-btn">结束</button>
                </div>
                ...
                <div class="chat-panel-input-row">
                    <textarea class="chat-panel-input" rows="2" placeholder="输入消息…"></textarea>
                    <div class="chat-panel-actions">
                        <label class="chat-panel-upload-label">
                            <input type="file" class="chat-panel-file" multiple hidden />
                            +
                        </label>
                        <button class="chat-panel-send-btn">发送</button>
                    </div>
                </div>
            </div>
        `;
    ...
    setConversation(convId, convType, employeeName) {
        ...
        const typeLabels = { oneonone: '1-on-1', ceo_session: '会话' };
        this._container.querySelector('.chat-panel-type').textContent =
            typeLabels[convType] || '对话';
        ...
        this._closeBtn.textContent = convType === 'ceo_session' ? '关闭' : '结束';
    }
