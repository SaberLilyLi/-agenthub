...
const STATUS_STYLES = {
    ...
    processing: { color: '#5e6ad2', label: '\u27f3 运行中' },
    completed:  { color: '#10b981', label: '\u2713 已完成' },
    accepted:   { color: '#10b981', label: '\u2713 已验收' },
    finished:   { color: '#10b981', label: '\u2713 已结束' },
    failed:     { color: '#ef4444', label: '\u2717 失败' },
    cancelled:  { color: '#62666d', label: '\u2014 已取消' },
    holding:    { color: '#8b5cf6', label: '\u23f8 暂停中' },
    blocked:    { color: '#f97316', label: '\u2298 已阻塞' },
};
...
function _wrapText(desc, maxChars, maxLines) {
    if (!desc) return [''];
    if (/[\u3400-\u9fff]/.test(desc)) {
        const chars = Array.from(desc.replace(/\n/g, ' '));
        const lines = [];
...
class TaskTreeRenderer {
    ...
    _paginateChildren(node, pageSize) {
        ...
        if (node.children.length > pageSize) {
            ...
            const moreNode = {
                id: `_more_${node.id}`,
                name: `另有 ${hidden.length} 项`,
                avatar: '···',
                avatarUrl: '',
                dept: 'Default',
                role: '',
                desc: `点击展开其余 ${hidden.length} 项任务`,
                status: 'pending',
                _isShowMore: true,
                _parentId: node.id,
    ...
    _expandMoreChildren(moreModel) {
        ...
        if (remaining.length > 0) {
            const newMore = {
                ...
                name: `另有 ${remaining.length} 项`,
                avatar: '···',
                avatarUrl: '',
                dept: 'Default',
                role: '',
                desc: `点击展开其余 ${remaining.length} 项任务`,
                status: 'pending',
                _isShowMore: true,
                ...
                if (logContent.classList.contains('hidden')) {
                    ...
                    if (typeof XTermLog !== 'undefined') {
                        ...
                        fetch(`/api/node/${nodeId}/logs${qs}`)
                            ...
                            .catch(() => { xterm.writeln(`${ANSI.red}日志加载失败${ANSI.reset}`); });
                        logContent._xterm = xterm;
                    } else {
                        logContent.innerHTML = '<div style="color:#666;padding:8px">终端未加载</div>';
                    }
                } else {
    ...
    _renderNodeDetail(node) {
        ...
        const acceptance = node.acceptance_result
            ? `<div class="detail-section">
                 ...
                 <span class="${node.acceptance_result.passed ? 'status-pass' : 'status-fail'}">
                   ${node.acceptance_result.passed ? '通过' : '未通过'}
                 </span>
        ...
        const nodeTypeLabel = node.node_type === 'ceo_prompt' ? '原始任务'
            : node.node_type === 'ceo_followup' ? '后续要求'
            : node.node_type === 'ceo_request' ? 'CEO 要求' : '';
        ...
        return `
            ...
            <div class="detail-section">
                <h4>任务提示</h4>
                <pre class="detail-prompt">${this._escapeHtml(node.description || '（无）')}</pre>
            </div>

            ${criteria ? `<div class="detail-section"><h4>验收标准</h4><ul>${criteria}</ul></div>` : ''}

            <div class="detail-section">
                <h4>执行结果</h4>
                <pre class="detail-result">${this._escapeHtml(node.result || '（待处理）')}</pre>
            </div>
            ...
            <div class="detail-section detail-meta">
                <span>Token：输入 ${node.input_tokens || 0} / 输出 ${node.output_tokens || 0}</span>
                <span>成本：$${(node.cost_usd || 0).toFixed(4)}</span>
                <span>超时：${node.timeout_seconds || 3600} 秒</span>
            </div>
    ...
    _renderDependencies(node) {
        ...
        if (node.depends_on && node.depends_on.length > 0) {
            ...
            html += '<p class="node-log-empty">无</p>';
        }
        html += '</div>';
        ...
        html += '<div class="detail-section"><h4>下游任务</h4>';
