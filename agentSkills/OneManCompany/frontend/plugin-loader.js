...
class PluginLoader {
  ...
  async render(pluginId, projectId, container, ctx) {
    ...
    if (!plugin) {
      container.innerHTML = `<div style="color:var(--pixel-red);font-size:6px;">未找到插件“${pluginId}”</div>`;
      return;
    }

    container.innerHTML = '<div style="color:var(--text-dim);font-size:6px;">正在加载…</div>';
    ...
    try {
      ...
      if (typeof renderFn !== 'function') {
        container.innerHTML = `<div style="color:var(--pixel-red);font-size:6px;">未找到渲染函数“${plugin.render_function}”</div>`;
        return;
      }

      renderFn(container, data, ctx);
    } catch (err) {
      container.innerHTML = `<div style="color:var(--pixel-red);font-size:6px;">插件错误：${err.message}</div>`;
    }
  }
