...
function renderTimeline(container, data, ctx) {
  ...
  if (timeline.length === 0) {
    container.innerHTML = '<div style="color:var(--text-dim);font-size:6px;padding:8px;">暂无包含时间信息的已分派任务</div>';
    return;
  }
  ...
  for (const t of timeline) {
    ...
    if (t.estimated_duration_min && t.estimated_duration_min > 0) {
      ...
      html += `<div class="timeline-bar timeline-bar--estimated" style="left:${left.toFixed(1)}%;width:${estWidth.toFixed(1)}%;background:${color};" title="预计：${t.estimated_duration_min} 分钟"></div>`;
    }
    ...
    const statusText = status === 'in_progress' ? '运行中' : (t.end ? durationStr : '待处理');
