...
class AppController {
  ...
  connect() {
    ...
    const root = (window.__OMC_ROOT__ || '').replace(/\/$/, '');
    const path = `${root}/ws`;
    // Prefer same-origin (works through reverse proxy). Fall back to direct
    // OMC port when Next.js rewrites do not upgrade WebSocket.
    const candidates = [`${protocol}//${location.host}${path}`];
    if (root && location.port && location.port !== '8001') {
      candidates.push(`${protocol}//${location.hostname}:8001${path}`);
    }
    this._wsCandidates = candidates;
    this._wsCandidateIndex = 0;
    this._openWebSocket(candidates[0]);
  }
  ...
  _openWebSocket(wsUrl) {
    ...
    this.ws.onopen = () => {
      ...
      if (banner) {
        ...
        if (applyBtn) { applyBtn.textContent = '应用'; applyBtn.disabled = false; }
      }
      this.bootstrap();
    };
    ...
    this.ws.onclose = () => {
      ...
      if (this._wsCandidateIndex < candidates.length - 1) {
        this._wsCandidateIndex += 1;
        setTimeout(() => this._openWebSocket(candidates[this._wsCandidateIndex]), 200);
        return;
      }
      this._wsCandidateIndex = 0;
  ...
  updateRoster(employees) {
    ...
    for (const emp of filtered) {
      ...
      const guidanceBadge = guidanceCount > 0
        ? `<span style="color: #aa66ff; font-size: 6px;">［${guidanceCount} 条备注］</span>`
        : '';
  ...
  _populateRosterFilters(employees) {
    ...
    roleSelect.innerHTML = '<option value="">全部岗位</option>' +
  ...
  _activityTypeMap = {
    ...
    'ceo_task': (e) => ({ agent: 'CEO', text: `任务：${e.task || ''}` }),
  ...
  bindUI() {
    ...
    document.getElementById('abort-all-toolbar-btn')?.addEventListener('click', async () => {
        if (!confirm('确定要停止全部员工的所有任务吗？\n这将取消全部员工正在运行的所有任务。')) return;
        try {
  ...
  async _startGroupMeeting(meetingType) {
    ...
    document.getElementById('oneonone-chat-title').textContent = `🎓 ${typeLabel}会议`;
  ...
  openEmployeeDetail(emp) {
    ...
    if (principles) {
      ...
      principlesEl.innerHTML = '<span class="empty-hint">暂未设置工作原则</span>';
    }
    ...
    if (notes.length > 0) {
      ...
      guidanceEl.innerHTML = '<span class="empty-hint">暂无 1-on-1 备注</span>';
    }
  ...
  async _fetchExecutionLogs(empId) {
    ...
    try {
      ...
      if (data.logs && data.logs.length > 0) {
        ...
        el.innerHTML = '<span class="empty-hint">暂无日志</span>';
      }
    } catch (err) {
  ...
  async _fetchProgressLog(empId) {
    try {
      ...
      if (entries.length > 0 && typeof XTermLog !== 'undefined') {
        ...
        this._empProgressXterm.writeln(`${ANSI.dim}── 工作记录（已完成任务摘要）──${ANSI.reset}`);
        for (const e of entries) {
        ...
        el.innerHTML = '<span class="empty-hint">暂无工作记录</span>';
      }
    } catch (err) {
  ...
  _renderTaskBoard(tasks, counts) {
    ...
    if (!tasks || tasks.length === 0) {
      el.innerHTML = tabs + '<span class="empty-hint">暂无任务</span>';
      return;
    }
    ...
    for (const task of tasks) {
      ...
      if (task.status === 'pending' || task.status === 'processing') {
        html += `<button class="emp-task-cancel-btn" onclick="window._abortAgentTask('${empId}','${task.id}')">取消</button>`;
      }
      html += `</div>`;
  ...
  openTraceViewer(projectId, projectName) {
    ...
    fetch(`/api/projects/${projectId}/tree`)
      ...
      .then(async data => {
        ...
        metaEl.textContent = `${Object.keys(nodes).length} 个节点`;
      })
      .catch(e => {
  ...
  async _fetchCronList(empId) {
    try {
      ...
      if (crons.length === 0) {
        container.innerHTML = '<span class="empty-hint">暂无定时任务</span>';
  ...
  async _cancelCron(empId, cronName) {
    if (!confirm(`停止定时任务“${cronName}”吗？其待处理任务也会被取消。`)) return;
    try {
  ...
  async _stopAllCrons(empId) {
    if (!confirm('停止该员工的全部定时任务吗？')) return;
    try {
  ...
  _fetchEmployeeProjects(employeeId) {
    ...
    if (!container) return;
    container.innerHTML = '<span class="empty-hint">正在加载...</span>';

    fetch(`/api/employees/${employeeId}/projects`)
      .then(r => r.json())
      .then(projects => {
        if (!projects || projects.length === 0) {
          container.innerHTML = '<span class="empty-hint">暂无项目历史</span>';
          return;
        }
        let html = '';
  ...
  _showCodeUpdateBanner(count, files) {
    ...
    textEl.textContent = `🔄 ${count} 个后端文件已更改：${shortFiles.slice(0, 3).join(', ')}${count > 3 ? '...' : ''}`;
  ...
  async _initCeoTerminal() {
    ...
    const doSend = async () => {
      ...
      if (this._pendingIterProject) {
        ...
        if (input) input.placeholder = '$ 输入消息，输入 / 可使用命令（按 Enter 发送）';
        try {
    ...
    input?.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        if (menuVisible) {
          ...
          this._pendingIterProject = null;
          this._pendingSimpleMode = false;
          input.placeholder = '$ 输入消息，输入 / 可使用命令（按 Enter 发送）';
  ...
  get _slashCommands() {
    ...
    return [
      { cmd: '/new', desc: '新建项目', action: (arg) => {
        if (arg) {
          ...
          fetch('/api/ceo/task', { method: 'POST', body: formData })
            .then(() => { this._refreshCeoProjectList(); this._ceoTerm?.appendMessage({ role: 'system', text: '✓ 项目已创建', source: 'system' }); })
            .catch(e => { this._ceoTerm?.appendMessage({ role: 'system', text: `✗ 失败：${e.message}`, source: 'system' }); });
        } else {
          // /new with no arg → switch to EA chat for input
          this._openEaChat();
        }
      }},
      { cmd: '/iter', desc: projName ? `为“${projName}”创建新一轮迭代` : '请先选择项目', action: (arg) => {
        if (!this._currentCeoProject || this._currentCeoProject === this._EA_CHAT) {
          this._ceoTerm?.appendMessage({ role: 'system', text: '请先选择项目，再使用 /iter', source: 'system' });
          return;
        }
        if (arg) {
          ...
          this._attachPendingFilesToFormData(formData);
          fetch('/api/ceo/task', { method: 'POST', body: formData })
            .then(() => { this._refreshCeoProjectList(); this._ceoTerm?.appendMessage({ role: 'system', text: '✓ 新一轮迭代已创建', source: 'system' }); })
            .catch(e => { this._ceoTerm?.appendMessage({ role: 'system', text: `✗ 失败：${e.message}`, source: 'system' }); });
        } else {
          // /iter with no arg → prompt for input
          this._pendingIterProject = this._currentCeoProject;
          this._ceoTerm?.appendMessage({ role: 'system', text: `输入“${projName}”的新一轮目标，按 Enter 创建。`, source: 'system' });
          const input = document.getElementById('ceo-conv-input');
          if (input) input.placeholder = `$ 为 ${projName} 提交新一轮要求...`;
        }
      }},
      { cmd: '/end', desc: this._currentConvType === 'meeting' ? '结束当前会议' : (this._currentConvType === 'oneonone' ? '结束 1-on-1' : '当前没有进行中的会话'), action: () => {
        if (this._currentConvType === 'meeting') {
          this._endMeetingInConsole();
        } else if (this._currentConvType === 'oneonone' && this._currentConvId) {
          this._endOneononeFromTerminal();
        } else {
          this._ceoTerm?.appendMessage({ role: 'system', text: '当前没有可结束的会议或 1-on-1。', source: 'system' });
        }
      }},
      { cmd: '/simple', desc: '简单任务（不做复盘）', action: (arg) => {
        if (arg) {
          ...
          this._attachPendingFilesToFormData(formData);
          fetch('/api/ceo/task', { method: 'POST', body: formData })
            .then(() => { this._refreshCeoProjectList(); this._ceoTerm?.appendMessage({ role: 'system', text: '✓ 简单任务已创建', source: 'system' }); })
            .catch(e => { this._ceoTerm?.appendMessage({ role: 'system', text: `✗ 失败：${e.message}`, source: 'system' }); });
        } else {
          ...
          this._ceoTerm?.appendMessage({ role: 'system', text: '简单任务模式：输入任务后按 Enter。', source: 'system' });
          const input = document.getElementById('ceo-conv-input');
          if (input) input.placeholder = '$ 简单任务（按 Enter 提交）...';
        }
      }},
      { cmd: '/review', desc: '发起季度绩效评审', action: () => {
        this._ceoTerm?.appendMessage({ role: 'system', text: '正在发起季度绩效评审...', source: 'system' });
        this.logEntry('CEO', '🔄 正在发起季度绩效评审...', 'ceo');
        fetch('/api/hr/review', { method: 'POST' })
          .then(r => r.json())
          .then(data => {
            if (data.error) {
              this._ceoTerm?.appendMessage({ role: 'system', text: `评审失败：${data.error}`, source: 'system' });
              this.logEntry('SYSTEM', `评审失败：${data.error}`, 'system');
            } else {
              this._ceoTerm?.appendMessage({ role: 'system', text: '📋 季度评审任务已分配给 HR', source: 'system' });
              this.logEntry('HR', '📋 季度评审任务已分配给 HR', 'hr');
            }
          })
          .catch(e => {
            this._ceoTerm?.appendMessage({ role: 'system', text: `评审出错：${e.message}`, source: 'system' });
          });
      }},
      { cmd: '/1on1', desc: '与员工开始 1-on-1 沟通', action: () => {
      ...
      { cmd: '/allhands', desc: '开始全员大会（CEO 讲话）', action: async (arg) => {
        await this._startMeetingInConsole('all_hands', arg);
      }},
      { cmd: '/discuss', desc: '开始开放讨论会议', action: async (arg) => {
        await this._startMeetingInConsole('discussion', arg);
      }},
      { cmd: '/clear', desc: '清空 EA 对话记录', action: async () => {
        if (this._currentCeoProject !== this._EA_CHAT) {
          this._ceoTerm?.appendMessage({ role: 'system', text: '/clear 只能在 EA 对话中使用。', source: 'system' });
          return;
        }
  ...
  async _loadModelOrApiKeySection(empId) {
    ...
    container.innerHTML = '<div style="color:var(--text-dim);font-size:6px;padding:4px;">正在加载设置...</div>';
    ...
    try {
      ...
      if (empResp.hosting === 'self') {
        ...
        if (empResp.level >= 4) {
          ...
          notice.textContent = '⚠ 修改设置会触发服务重载，请在没有运行中任务时操作。';
        ...
        saveRow.innerHTML = '<button class="pixel-btn small" id="emp-manifest-save-btn">保存</button>';
      ...
      container.innerHTML = '<div style="color:var(--pixel-red);font-size:6px;">加载失败</div>';
    }
  }
  ...
  _renderManifestField(field, empData, empId) {
    ...
    if (field.type === 'secret') {
      ...
      status.textContent = isSet ? '已设置' : '无';
      ...
      btn.textContent = empData.oauth_logged_in ? '重新登录' : '登录';
      ...
      const btn = document.createElement('button');
      btn.className = 'pixel-btn small';
      btn.textContent = field.label || '运行';
      btn.dataset.fieldKey = field.key;
  ...
  async _populateModelSelect(select, currentModel) {
    try {
      ...
      select.innerHTML = '<option value="">-- 使用默认值 --</option>';
  ...
  _manifestActions = {
    hire_from_cv: async (field, empId, sectionEl) => {
      ...
      try {
        ...
        btn.disabled = false;
        btn.textContent = field.label || '录用';
      }
    },
  };
    ...
    fetch(`/api/employee/${empId}/api-key`, {
      ...
      .then(data => {
        if (data.error) {
          ...
          keyStatus.textContent = data.api_key_set ? '已认证' : '未配置密钥';
          keyStatus.style.color = data.api_key_set ? 'var(--pixel-green)' : 'var(--pixel-red, #f44)';
          keyInput.value = '';
          keyInput.placeholder = data.api_key_set ? 'API 密钥已设置（如需更改请重新输入）' : '请输入 API 密钥…';
  ...
  async _refreshSessionStatus(empId, empData) {
    ...
    try {
      ...
      if (sessions.length > 0) {
        ...
        statusEl.textContent = `会话（${sessions.length}）：${labels}`;
        statusEl.style.color = 'var(--pixel-green)';
        keyStatus.textContent = `${sessions.length} 个会话`;
        keyStatus.style.color = 'var(--pixel-green)';
      } else {
        statusEl.textContent = '按需启动（当前无会话）';
        statusEl.style.color = 'var(--pixel-blue, #4af)';
        keyStatus.textContent = '就绪';
        keyStatus.style.color = 'var(--pixel-green)';
      }
    } catch {
  ...
  async showCandidateSelection(payload) {
    ...
    // JD sidebar
    jdEl.innerHTML = '<div style="font-size:7px;color:var(--pixel-yellow);margin-bottom:4px;">JD — 职位说明</div>' +
      (payload.jd || '').replace(/\n/g, '<br>');
    ...
    for (const roleGroup of this._candidateRoles) {
      ...
      for (const c of (roleGroup.candidates || [])) {
        ...
        const warningBadge = llmWarning
          ? `<div class="card-config-warning" title="${esc(`公司默认：${llmWarning.companyProvider}${llmWarning.companyModel ? ` / ${llmWarning.companyModel}` : ''}\n人才偏好：${llmWarning.talentProvider}${llmWarning.talentModel ? ` / ${llmWarning.talentModel}` : ''}`)}">⚠ 与公司默认配置不同</div>`
          : '';
  ...
  _showCandidateDetail(candidateId, candidate, role, cardEl) {
    ...
    const warningSection = llmWarning ? `
      <div class="detail-section">
        <div class="detail-label">大模型配置</div>
        <div class="detail-config-warning-box">
          <div>公司统一配置：<strong>${esc(llmWarning.companyProvider)}${llmWarning.companyModel ? ` / ${esc(llmWarning.companyModel)}` : ''}</strong></div>
          <div>人才推荐配置：<strong>${esc(llmWarning.talentProvider)}${llmWarning.talentModel ? ` / ${esc(llmWarning.talentModel)}` : ''}</strong></div>
          <div class="detail-config-warning-note">招聘后将统一使用公司的大模型配置，无需为新员工单独填写 API Key。</div>
        </div>
      </div>` : '';
    ...
    selectBtn.textContent = isSelected ? '✗ 取消选择' : '✔ 选择';
    ...
    newSelectBtn.textContent = isSelected ? '✗ 取消选择' : '✔ 选择';
    ...
    if (!isRemote) {
      ...
      newInterviewBtn.textContent = '🔒 面试';
    } else {
    ...
    newSelectBtn.addEventListener('click', () => {
      ...
      newSelectBtn.textContent = nowSelected ? '✗ 取消选择' : '✔ 选择';
  ...
  _updateBatchBar() {
    ...
    if (count > 0) {
      ...
      countEl.textContent = talentConfigCount > 0
        ...
        : `${count} selected`;
      btn.textContent = `批量招聘（${count}）`;
      btn.disabled = false;
    } else {
      bar.classList.remove('hidden'); // always show bar for context
      countEl.textContent = '已选 0 人 — 点击候选人卡片进行选择';
      btn.textContent = '批量招聘（0）';
      btn.disabled = true;
    }
  }
  ...
  hireCandidate(candidate) {
    ...
    document.querySelectorAll('.pixel-btn.hire').forEach(b => { b.disabled = true; b.textContent = '正在录用...'; });
    ...
    fetch('/api/candidates/hire', {
      ...
      .then(data => {
        if (data.error) {
          ...
          document.querySelectorAll('.pixel-btn.hire').forEach(b => { b.disabled = false; b.textContent = '录用'; });
        } else {
  ...
  loadProjectList() {
    ...
    listEl.innerHTML = '<div style="color:var(--text-dim);font-size:7px;">正在加载...</div>';
    ...
    fetch('/api/projects/named')
      ...
      .catch(err => {
        listEl.innerHTML = `<div style="color:var(--pixel-red);font-size:7px;">加载失败：${this._escHtml(err.message)}</div>`;
      });
  }
  ...
  _showProjectInModal(projectId) {
    ...
    contentEl.innerHTML = '<div style="color:var(--text-dim);font-size:7px;">正在加载...</div>';
    ...
    fetch(`/api/projects/named/${encodeURIComponent(projectId)}`)
      ...
      .catch(err => {
        contentEl.innerHTML = `<div style="color:var(--pixel-red);font-size:7px;">加载失败：${this._escHtml(err.message)}</div>`;
      });
  }
    ...
    fetch(`/api/projects/${encodeURIComponent(projectId)}`)
      ...
      .then(doc => {
        ...
        if (cost.actual_cost_usd > 0 || cost.budget_estimate_usd > 0) {
          html += `<div style="font-size:7px;color:var(--pixel-cyan);margin:8px 0 4px;">成本与预算：<span style="font-size:5px;color:var(--text-dim);">（估算）</span></div>`;
          ...
          html += `<div style="font-size:6px;color:var(--pixel-white);margin:2px 0;">实际：$${actual.toFixed(4)}${budgetLine}</div>`;
          html += `<div style="font-size:6px;color:var(--text-dim);margin:2px 0;">Token：输入 ${(tokens.input||0).toLocaleString()} / 输出 ${(tokens.output||0).toLocaleString()}</div>`;
          // Breakdown table
          const breakdown = cost.breakdown || [];
          if (breakdown.length > 0) {
            html += `<table style="font-size:5px;width:100%;border-collapse:collapse;margin-top:4px;">`;
            html += `<tr style="color:var(--text-dim);"><th style="text-align:left;">员工</th><th>模型</th><th>Token</th><th>成本</th></tr>`;
        ...
        html += `<div style="font-size:7px;color:var(--pixel-cyan);margin:8px 0 4px;">文档：</div>`;
      ...
      .catch(err => {
        contentEl.innerHTML = `<div style="color:var(--pixel-red);">加载失败：${this._escHtml(err.message)}</div>`;
      });
  }
  ...
  openPopup(opts = {}) {
    ...
    title.textContent = opts.title || '通知';
    body.innerHTML = '';
    footer.innerHTML = '';
    ...
    if (type === 'confirm') {
      ...
      confirmBtn.textContent = opts.confirm_label || '确认';
      confirmBtn.onclick = () => {
    ...
    if (type === 'credentials' && opts.fields) {
      ...
      submitBtn.textContent = opts.submit_label || '提交';
      submitBtn.onclick = () => {
        const values = {};
    ...
    fetch(`/api/workflows/${encodeURIComponent(this.currentWorkflowName)}`, {
      ...
      .then(data => {
        if (data.error) {
          ...
          document.getElementById('workflow-content').classList.add('hidden');
          rendered.classList.remove('hidden');
          document.getElementById('workflow-edit-btn').textContent = '✎ 编辑';
          document.getElementById('workflow-save-btn').classList.add('hidden');
        }
      })
  ...
  openMeetingRoom(room) {
    ...
    chatEl.innerHTML = '<div class="chat-empty">正在加载...</div>';
    fetch(`/api/rooms/${encodeURIComponent(room.id)}/chat`)
      ...
      .catch(err => {
        ...
        chatEl.innerHTML = '<div class="chat-empty">聊天记录加载失败</div>';
      });
  }
  ...
  openMeetingMinutes(room) {
    const modal = document.getElementById('meeting-modal');
    modal.classList.remove('hidden');
    document.getElementById('meeting-modal-title').textContent = `会议纪要：${room.name}`;

    // Reuse meeting modal body area for minutes list
    const chatEl = document.getElementById('meeting-chat-messages');
    chatEl.innerHTML = '<div class="chat-empty">正在加载会议纪要...</div>';
    ...
    fetch(`/api/rooms/${encodeURIComponent(room.id)}/minutes`)
      ...
      .then(minutes => {
        if (!minutes || minutes.length === 0) {
          chatEl.innerHTML = '<div class="chat-empty">暂无已归档会议</div>';
          return;
        }
        chatEl.innerHTML = '';
      ...
      .catch(err => {
        ...
        chatEl.innerHTML = '<div class="chat-empty">会议纪要加载失败</div>';
      });
  }
    ...
    try {
      ...
      list.innerHTML = items.map(a => `
        <div class="announcement-item" data-id="${a.id}">
          <button class="announcement-dismiss" title="关闭" onclick="window.app._dismissAnnouncement(${a.id})">&times;</button>
      ...
      for (const group of groups) {
        ...
        const oauthSection = hasSetupToken ? `
              ...
              <div style="border-top:1px solid var(--border);padding-top:4px;margin-top:4px;">
                <label class="api-field-label" style="font-size:5.5px;color:var(--text-dim);">或直接使用 API 密钥</label>
              </div>` : '';
          ...
          <div class="api-provider-card${isDefault ? ' is-default' : ''}">
            ...
            <div id="${bodyId}" class="api-card-body collapsed">
              ...
              <div style="margin-top:6px;border-top:1px solid var(--border);padding-top:6px;">
                <label class="api-field-label">默认模型</label>
  ...
  _renderDashboard() {
    ...
    content.innerHTML = `
      <div class="dash-section">
        <div class="dash-title">员工概览</div>
        <div class="dash-stats">
          <div class="dash-stat"><span class="dash-num">${employees.length}</span><span class="dash-label">在职</span></div>
          <div class="dash-stat"><span class="dash-num">${exEmployees.length}</span><span class="dash-label">已离职</span></div>
      ...
      <div class="dash-section">
        <div class="dash-title">工具与会议室</div>
        <div class="dash-stats">
      ...
      <div class="dash-section">
        ...
        <div class="dash-stats">
          <div class="dash-stat"><span class="dash-num">${tasks.filter(t => t.status === 'running').length}</span><span class="dash-label">运行中</span></div>
    ...
    fetch('/api/dashboard/costs').then(r => r.json()).then(data => {
      ...
      costHtml += `
        <div class="dash-section">
          <div class="dash-title">\u{1F4B0} 成本概览 <span style="font-size:5px;color:var(--text-dim);">（估算值，以实际账单为准）</span></div>
          <div class="dash-stats">
            <div class="dash-stat"><span class="dash-num" style="color:var(--pixel-yellow);">$${grandTotal.toFixed(3)}</span><span class="dash-label">总成本</span></div>
            <div class="dash-stat"><span class="dash-num">$${projectCost.toFixed(3)}</span><span class="dash-label">项目</span></div>
            <div class="dash-stat"><span class="dash-num">$${overheadCost.toFixed(3)}</span><span class="dash-label">管理开销</span></div>
          </div>
          <div class="dash-stats" style="margin-top:4px;">
            <div class="dash-stat"><span class="dash-num">${((t.total_tokens || 0) / 1000).toFixed(1)}k</span><span class="dash-label">项目 Token</span></div>
            <div class="dash-stat"><span class="dash-num">${(((oh.total_input_tokens || 0) + (oh.total_output_tokens || 0)) / 1000).toFixed(1)}k</span><span class="dash-label">管理开销 Token</span></div>
          </div>
        </div>`;
      ...
      if (Object.keys(cats).length) {
        const catLabels = {oneonone:'1-on-1', meeting:'会议', routine:'例行任务', interview:'面试', agent_task:'智能体任务', history_compress:'历史压缩', completion_check:'完成检查', nickname_gen:'昵称生成', remote_worker:'远程员工'};
        costHtml += `
          <div class="dash-section">
            <div class="dash-title">\u{1F4B0} 分类管理开销</div>
            <table class="dash-cost-table">
              <tr><th>类别</th><th>USD</th><th>输入 Token</th><th>输出 Token</th></tr>
      ...
      if (Object.keys(deptCosts).length) {
        costHtml += `
          <div class="dash-section">
            <div class="dash-title">\u{1F4B0} 部门成本</div>
            <table class="dash-cost-table">
              <tr><th>部门</th><th>USD</th><th>Token</th></tr>
      ...
      if (projects.length) {
        costHtml += `
          <div class="dash-section">
            <div class="dash-title">\u{1F4B0} 近期项目成本</div>
            <table class="dash-cost-table">
              <tr><th>项目</th><th>USD</th><th>Token</th><th>状态</th></tr>
              ${projects.map(p =>
          ...
          return `
            <div class="company-culture-card">
              ...
              <div class="company-culture-card-meta">
                ...
                <button class="company-culture-delete-btn" data-index="${idx}" title="删除">✕</button>
              </div>
            </div>`;
        }).join('');
        // Bind delete buttons
    ...
    fetch('/api/products/panel')
      ...
      .then(data => {
        ...
        if (products.length === 0 && orphans.length === 0) {
          panel.innerHTML = '<div class="task-empty">暂无产品或项目</div>';
          return;
        }
  ...
  _renderProjectDetail(projectId, proj, contentEl) {
    ...
    let headerHtml = `<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;">
      <span class="project-name-editable" data-project-id="${this._escHtml(projectId)}" title="点击重命名" style="color:var(--pixel-cyan);font-size:8px;cursor:pointer;border-bottom:1px dashed var(--text-dim);">${this._escHtml(proj.name || projectId)}</span>
      ...
      <button onclick="app.openTraceViewer('${this._escHtml(projectId)}','${this._escHtml(proj.name || projectId)}')" style="margin-left:auto;background:#1a1a1a;color:#4af;border:1px solid #333;padding:1px 8px;font-size:6px;cursor:pointer;font-family:monospace">追踪</button>
      <button onclick="app._deleteProject('${this._escHtml(projectId)}')" style="background:#1a1a1a;color:var(--pixel-red);border:1px solid #500;padding:1px 8px;font-size:6px;cursor:pointer;font-family:monospace" title="删除项目及其全部数据">删除</button>
    </div>`;
    ...
    if (proj.status === 'active') {
      iterListHtml += `<div style="margin-top:8px;"><button class="pixel-btn secondary archive-project-btn" style="font-size:6px;padding:4px 8px;">归档</button></div>`;
    }
  ...
  _loadIterationDetail(projectId, iterationId) {
    ...
    Promise.all([
      ...
      fetch(`/api/projects/${qualifiedPath}/tree`).then(r => r.ok ? r.json() : null).catch(() => null),
        ...
        detailHtml += `<div style="font-size:5px;color:var(--text-dim);margin-bottom:8px;">状态：${({in_progress:'进行中',completed:'已完成',archived:'已归档',failed:'失败',pending:'待处理'})[doc.status] || doc.status} | 负责人：${doc.current_owner || '-'} | ID：${this._escHtml(qualifiedId || projectId)}</div>`;
        ...
        if (doc.status !== 'completed' && doc.status !== 'pending_confirmation') {
          ...
          detailHtml += `<button class="pixel-btn" id="continue-iter-btn" style="font-size:6px;padding:4px 10px;">\u25B6 继续当前迭代</button>`;
          detailHtml += `<button class="pixel-btn" id="stop-iter-btn" style="font-size:6px;padding:4px 10px;background:var(--pixel-red);color:#000;">■ 停止全部任务</button>`;
          detailHtml += `</div>`;
        }

        // Follow-up button (always available)
        detailHtml += `<div class="task-followup-section">
          <button class="pixel-btn" id="followup-btn" style="font-size:6px;padding:4px 10px;">+ 后续任务</button>
        ...
        detailHtml += `<div style="font-size:7px;color:var(--pixel-cyan);margin:6px 0 3px;display:flex;justify-content:space-between;align-items:center;">
          <span>文档</span>
          <a href="${downloadUrl}" style="font-size:5px;color:var(--pixel-green);text-decoration:none;border:1px solid var(--border);padding:1px 6px;cursor:pointer;">下载 ZIP</a>
        </div>`;
        ...
        const timeline = doc.timeline || [];
        detailHtml += `<div style="font-size:7px;color:var(--pixel-cyan);margin:8px 0 3px;">日志（${timeline.length}）</div>`;
        if (timeline.length > 0) {
        ...
        const cost = doc.cost || {};
        detailHtml += `<div style="font-size:7px;color:var(--pixel-cyan);margin:8px 0 3px;">成本与预算 <span style="font-size:5px;color:var(--text-dim);">（估算）</span></div>`;
        const actual = cost.actual_cost_usd || 0;
        const budget = cost.budget_estimate_usd || 0;
        const tokens = cost.token_usage || {};
        if (actual > 0 || budget > 0) {
          ...
          detailHtml += `<div style="font-size:6px;color:var(--pixel-white);margin:2px 0;">实际：$${actual.toFixed(4)}${budgetLine}</div>`;
          detailHtml += `<div style="font-size:5px;color:var(--text-dim);margin:2px 0;">Token：输入 ${(tokens.input||0).toLocaleString()} / 输出 ${(tokens.output||0).toLocaleString()}</div>`;
          const breakdown = cost.breakdown || [];
          if (breakdown.length > 0) {
            detailHtml += `<table style="font-size:5px;width:100%;border-collapse:collapse;margin-top:3px;">`;
            detailHtml += `<tr style="color:var(--text-dim);"><th style="text-align:left;">员工</th><th>模型</th><th>Token</th><th>成本</th></tr>`;
            for (const b of breakdown) {
        ...
        if (stopBtn) {
          stopBtn.addEventListener('click', () => {
            ...
            fetch(`/api/task/${encodeURIComponent(iterationId)}/abort`, { method: 'POST' })
              ...
              .catch(err => { console.error('[stopTasks] failed:', err); stopBtn.disabled = false; stopBtn.textContent = '■ 停止全部任务'; });
          });
        }
    ...
    fetch('/api/projects/continue', {
      ...
      .then(data => {
        if (data.error) {
          ...
          if (btn) { btn.disabled = false; btn.textContent = '▶ 继续当前迭代'; }
        } else {
  ...
  _deleteProject(projectId) {
    if (!confirm(`确定删除项目“${projectId}”及其全部数据吗？此操作无法撤销。`)) return;
