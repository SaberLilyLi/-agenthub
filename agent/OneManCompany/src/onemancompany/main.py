...
# Rebuild settings after dotenv so OMC_ROOT_PATH / PORT from .env are applied
from onemancompany.core import config as _config_mod  # noqa: E402
...
from onemancompany.core.config import normalize_root_path, settings  # noqa: E402
...
async def lifespan(app: FastAPI):
    ...
    for emp_id, emp_data in _store_mod.load_all_employees().items():
        ...
        # Only employees explicitly configured for OpenClaw may use launch.sh.
        # Talent packages can leave a legacy launch.sh behind; company-hosted
        # employees must still run through EmployeeAgent so they inherit the
        # company's configured LLM provider and credentials.
        _launch_sh = _emp_dir / LAUNCH_SH_FILENAME
        if _cfg and _cfg.hosting == HostingMode.OPENCLAW and _launch_sh.exists():
        ...
        if _launch_sh.exists() and (not _cfg or _cfg.hosting == HostingMode.COMPANY):
            logger.info(
                "[startup] Ignoring legacy launch.sh for company-hosted {} ({}); using the company LLM stack",
                emp_data.get('name', emp_id),
                emp_id,
            )
...
def _build_app() -> FastAPI:
    """Create the core FastAPI app (API + WebSocket + static frontend)."""
    application = FastAPI(
...
def _mount_at_root_path(inner: FastAPI, root_path: str) -> FastAPI:
    """Expose ``inner`` under a public prefix such as ``/oneManCompany``."""
    from fastapi.responses import RedirectResponse
    from starlette.types import ASGIApp, Receive, Scope, Send

    class _RelativeRedirectMiddleware:
        """Rewrite absolute backend Location headers to path-only (proxy-safe)."""
    ...
    gateway = FastAPI(title="One Man Company Gateway", redirect_slashes=False)
    gateway.add_middleware(_RelativeRedirectMiddleware)

    @gateway.api_route(root_path, methods=["GET", "HEAD"])
    async def _prefix_no_slash() -> RedirectResponse:
        return RedirectResponse(url=f"{root_path}/", status_code=307)

    @gateway.get("/")
    async def _root_redirect() -> RedirectResponse:
        return RedirectResponse(url=f"{root_path}/", status_code=307)

    gateway.mount(root_path, inner)
    return gateway


_core_app = _build_app()
_root_path = normalize_root_path(settings.omc_root_path)
app = _mount_at_root_path(_core_app, _root_path) if _root_path else _core_app


def run() -> None:
    app.state.port = settings.port
    root = normalize_root_path(settings.omc_root_path)
    public_path = f"{root}/" if root else "/"
    print(f"Frontend: http://localhost:{settings.port}{public_path}")
    uvicorn.run(
