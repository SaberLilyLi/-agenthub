...
async def ceo_submit_task(
    ...
    if loop:
        ...
        try:
            ...
            await _conv_svc.push_system_message(_conv.id, f"项目已创建：{task[:100]}", source_employee="system")
...
def _fill_talent_defaults(talent_data: dict, *, use_talent_llm_config: bool = False) -> None:
    ...
    Company-hosted hires always use the company's provider/model so every new
    employee runs on the configured company stack. ``use_talent_llm_config`` is
    retained for API compatibility but is intentionally ignored for managed
    employees. Self-hosted and remote employees manage their own runtime.
    """
    ...
    if use_talent_llm_config:
        logger.info(
            "[hiring] Ignoring talent-specific LLM config for company-hosted talent; company defaults are enforced"
        )
