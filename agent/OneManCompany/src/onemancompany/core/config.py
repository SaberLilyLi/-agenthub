...
ENV_OMC_ROOT_PATH = "OMC_ROOT_PATH"
IS_DEBUG = os.environ.get(ENV_OMC_DEBUG, "0") == "1"


def normalize_root_path(raw: str | None) -> str:
    """Normalize a public URL prefix (e.g. ``/oneManCompany``).

    Empty / ``/`` means the app is served at the domain root.
    """
    value = (raw or "").strip()
...
class Settings(BaseSettings):
    ...
    host: str = "0.0.0.0"
    port: int = 8000
    # Public URL prefix when mounted behind AgentHub (e.g. /oneManCompany)
    omc_root_path: str = ""
    hr_review_interval_seconds: int = 300
    onboarding_timestamp: str = ""  # ISO 8601 timestamp of initial onboarding

    @model_validator(mode="after")
    def _normalize_omc_root_path(self) -> Settings:
        self.omc_root_path = normalize_root_path(self.omc_root_path)
        return self
